...
for energy in energies_J:
    plt.plot(angles, electron_recoil_angle(energy, angles), label=f'E={energies[i]}keV')
    i += 1

# Lables and title
plt.xlabel('Photon Scattering Angle (θ) /deg')
plt.ylabel('Electron Recoil Angle (ϕ)')
plt.title('Electron Recoil Angle vs Photon Scattering Angle')
plt.legend()
